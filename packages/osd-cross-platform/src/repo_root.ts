...
export const getRepoRoot = (path: string) => {
  // We can only find the appropriate root if an absolute path was given
  if (path && isAbsolute(path)) {
    if (path.startsWith(REPO_ROOT)) return REPO_ROOT;
    if (path.startsWith(REPO_ROOT_8_3)) return REPO_ROOT_8_3;
  }
...
export const relativeToRepoRoot = (path: string) => {
  ...
  return root ? relative(root, path) : null;
};
